...
public class PointLine {
	...
	void updateData(){
		...
		float ndpt1_mpt_dstx = (float)(mouse_point.getX() - ln1_endpoint1.getX());
		float ndpt1_mpt_dsty = (float)(mouse_point.getY() - ln1_endpoint1.getY());
		float ndpt1_mpt_dst = (float)Math.sqrt(ndpt1_mpt_dstx * ndpt1_mpt_dstx + ndpt1_mpt_dsty * ndpt1_mpt_dsty);
		
		float ndpt2_mpt_dstx = (float)(mouse_point.getX() - ln1_endpoint2.getX());
		float ndpt2_mpt_dsty = (float)(mouse_point.getY() - ln1_endpoint2.getY());
		float ndpt2_mpt_dst = (float)Math.sqrt(ndpt2_mpt_dstx * ndpt2_mpt_dstx + ndpt2_mpt_dsty * ndpt2_mpt_dsty);
		
		float dst_sum = ndpt1_mpt_dst + ndpt2_mpt_dst;
		
		float offset = 0.1f;
		
		if(dst_sum >= ln1_dst-offset && 
		   dst_sum <= ln1_dst+offset) isColliding = true;
	...
	void drawObjects(Graphics2D g2d){
		...
		g2d.drawLine((int)ln1_endpoint1.getX(),(int)ln1_endpoint1.getY(),
		             (int)mouse_point.getX(),(int)mouse_point.getY());
										
		g2d.setPaint(Color.BLUE);
		g2d.drawLine((int)ln1_endpoint2.getX(),(int)ln1_endpoint2.getY(),
		             (int)mouse_point.getX(),(int)mouse_point.getY());
